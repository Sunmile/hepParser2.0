...
def get_n_label(match_list, candidate_sort_index, n):
    ...
    for i in range(n):
        ...
        tmp_score = np.round(sorted_comp[id][3]/max_score,4)
        flag = [tmp_mass - 1, 0, []]
        ...
        for j in range(len(max_mass)):
            ...
            for i in (range(len(all_score_mass[j]))):
                ...
                if dict_exp_match[mass]>0:
                    ...
                    order_peak = i+1
                    break
            tmp_score_p = np.round(all_score_list[j]/max_score*10,4)
...
def get_most_power_candidate_score(match_list):
    ...
    while len(has_added_candidate) < n:
        if len(has_added_candidate) == 0:
            ...
            best_index = get_most_power_candidate3(sorted_comp, has_considered_peak, has_considered_peak, dict_score,dict_exp)
        if best_index >= 0:
...
def get_n_result(match_result, cand_num):
    result2_score, candidate_sort_index = get_most_power_candidate_score(match_result)
    label, dict_mass_comp, dict_mass_flag, dict_mass_family, key_with_order = get_n_label(match_result, candidate_sort_index, cand_num)
